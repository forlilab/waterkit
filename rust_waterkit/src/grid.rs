...
// impl Grid3D {
//     pub fn update_energies(&mut self, new_points: &Vec<Atom>, cutoff: Option<f64>) { 
//         if cutoff.is_some() {
//             // I include only the oxygen and add 1A that holds for the hydrogens
//             let c = cutoff.unwrap();
//             let indices = self.get_neighbors_within_distance_indices(&new_points[0].coords(), c);
//             let mut_points = self.all_points_mut();
//             for idx in indices {
//                 let (oda_energy, ow_energy, q_energy) = energy::update_grid_energies(new_points, &mut_points[idx].coords);
//                 mut_points[idx].energy_oda += oda_energy;
//                 mut_points[idx].energy_ow += ow_energy;
//                 mut_points[idx].energy_hw += q_energy;
//             }
//         } else {
//             for point in self.all_points_mut() {
//                 let (oda_energy, ow_energy, q_energy) = energy::update_grid_energies(new_points, &point.coords);
...
//     pub fn remove_points(&mut self, points_to_remove: &Vec<Atom>, cutoff: Option<f64>) {
//         if cutoff.is_some() {
//             // I include only the oxygen and add 1A that holds for the hydrogens
//             let c = cutoff.unwrap();
//             let indices = self.get_neighbors_within_distance_indices(&points_to_remove[0].coords(), c);
//             let mut_points = self.all_points_mut();
//             for idx in indices {
//                 let (oda_energy, ow_energy, q_energy) = energy::update_grid_energies(points_to_remove, &mut_points[idx].coords);
//                 mut_points[idx].energy_oda -= oda_energy;
//                 mut_points[idx].energy_ow -= ow_energy;
//                 mut_points[idx].energy_hw -= q_energy;
//             }
...
//     pub fn get_neighbors_within_distance_indices(
//         &self,
//         query_point: &[f64; 3],
//         max: f64,
//     ) -> Vec<usize> {
//         let max_distance = max.powf(2.0);
...
//         for (_distance, &index) in &within_max_distance {
//             let coords = self.data[index].coords;
//             if !self.is_close_to_edge(&[coords], 1.0) {
