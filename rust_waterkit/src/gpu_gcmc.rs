...
fn run_gcmc(
    ...
    for epoch in 0..epochs {
        ...
        let base_idx_for_rng = (sim_id * epochs + epoch) * 12;
        let move_type = 1.0;
        let base_water_idx = waters_base_idx + (active_waters as u32 * ATOM_FEATURES * 3);
        // if move_type == 1.0 {
        //     // debug
        ...
        // if move_type == 1.0 { // INSERTION
            let accepted_insertion = gpu_gcmc_moves::insertion_move(
                boundaries,
                receptor_atoms,
                water_atoms,
                random_numbers,
...
pub fn simulate<R: Runtime>(
    ...
    let energies_handle = client.empty(((n_receptor_atoms + max_n_waters * 3) * n_simulations) * core::mem::size_of::<f32>());
    let energies_debug = client.empty(n_simulations * num_steps * core::mem::size_of::<f32>());
    ...
    unsafe {
        run_gcmc::launch_unchecked::<R>(
            ...
            ArrayArg::from_raw_parts::<f32>(&energies_handle, (n_receptor_atoms + max_n_waters * 3) * n_simulations, 1), 
        ...
        ArrayArg::from_raw_parts::<f32>(&energies_debug, n_simulations * num_steps, 1));
    }
