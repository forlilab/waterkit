...
pub fn run_gcmc_multithread(
    ...
    rec_partials: &mut Array<f32>,
    wat_partials: &mut Array<f32>,
    last_resnum: u32,
    B: f32,
    volume: f32,
    steps: u32,
    ...
    for _step in 0..steps {
        ...
        if UNIT_POS_X == 0 {
            ...
            if active_waters == 0 {
                move_type = 0;
            } else if active_waters >= consts::MAX_N_WATERS {
        ...
        if UNIT_POS_X == 0 {
            ...
            if move_type == 0 {
                ...
                let e_wat_new = gpu_gcmc_moves::energy_for_real_water_with_waters_kernel(
                    water_atoms, &trial_water, sim_id, active_waters
                );
                ...
                let e_wat_old = gpu_gcmc_moves::energy_for_real_water_with_waters_kernel(
                    water_atoms, &old_copy, sim_id, active_waters
                );
                let e_wat_new = gpu_gcmc_moves::energy_for_real_water_with_waters_kernel(
                    water_atoms, &trial_water, sim_id, active_waters
                );
...
//     // Local scratch arrays for single-thread operations
//     let mut random_numbers = Array::new(12u32);
//     let mut trial_water = Array::new(12u32);
//     // Shared workgroup buffer to broadcast the trial water to all threads
//     let mut sh_trial = SharedMemory::<f32>::new(consts::WATER_SIZE);
//     let mut move_type = 0u32;

//     // SHARED MEMORY: Use shared memory to broadcast active_waters to all threads
//     // This ensures all threads see the same active_waters value
//     let mut shared_active_waters = SharedMemory::<u32>::new(1);
//     if UNIT_POS_X == 0 {
//         shared_active_waters[0] = active_waters;
//     }
...
//         // ===========================
//         if UNIT_POS_X == 0 {
//             // Store current active_waters in shared memory for all threads to see
//             shared_active_waters[0] = active_waters;
            
//             // Generate all random numbers needed for this step
//             random_numbers[consts::INSERTION_X_IDX] = gpu_random::random_range(&mut rng_state, boundaries[0], boundaries[1]);
//             random_numbers[consts::INSERTION_Y_IDX] = gpu_random::random_range(&mut rng_state, boundaries[2], boundaries[3]);
//             random_numbers[consts::INSERTION_Z_IDX] = gpu_random::random_range(&mut rng_state, boundaries[4], boundaries[5]);
//             random_numbers[consts::TRANSLATION_X_IDX] = gpu_random::random_range(&mut rng_state, -0.3f32, 0.3f32);
...
//             // Pick target water index if needed
//             if active_waters > 0 {
//                 random_numbers[consts::WATER_TARGET_IDX] = gpu_random::random_int_range(&mut rng_state, active_waters) as f32;
//             } else {
//                 random_numbers[consts::WATER_TARGET_IDX] = 0.0f32;
...
//             // Choose move type (0 = insertion, 1 = deletion)
//             if active_waters < consts::MAX_N_WATERS {
//                 move_type = gpu_random::random_int_range(&mut rng_state, 2);
//                 if active_waters == 0 {
//                     move_type = 0; // Force insertion if no waters
//                 }
//             }
//             // Prepare trial water based on move type
//             if move_type == 0 {
//                 let counter_idx = sim_id;
//                 let current_res = num_waters[counter_idx];
//                 let possible_resnum = current_res + 1;
...
//                 // Copy manually
//                 trial_water[0] = new_water[0];
//                 trial_water[1] = new_water[1];
//                 trial_water[2] = new_water[2];
//                 trial_water[3] = possible_resnum as f32;
//                 trial_water[4] = new_water[4];
//                 trial_water[5] = new_water[5];
//                 trial_water[6] = new_water[6];
//                 trial_water[7] = possible_resnum as f32;
//                 trial_water[8] = new_water[8];
//                 trial_water[9] = new_water[9];
//                 trial_water[10] = new_water[10];
//                 trial_water[11] = possible_resnum as f32;
//             } else if move_type == 1 {
//                 let waters_handle_idx = sim_id * consts::MAX_N_WATERS * consts::WATER_SIZE;
//                 let del_idx = random_numbers[consts::WATER_TARGET_IDX] as u32;
//                 let src = waters_handle_idx + del_idx * consts::WATER_SIZE;
//                 gpu_gcmc_moves::load_water_from_array(water_atoms, src, &mut trial_water);
//             }
//             // Broadcast trial_water to shared memory for all threads
//             for i in 0..consts::WATER_SIZE { sh_trial[i] = trial_water[i]; }
//         }

//         // Synchronize before energy calculations - this broadcasts shared_active_waters
//         sync_cube();
        
//         // ALL THREADS: Read the current active_waters from shared memory
//         let current_active_waters = shared_active_waters[0];

//         // ==================================
//         // MULTITHREAD: compute trial energy
//         // ==================================
//         // Synchronize to ensure sh_trial is fully written by thread 0
//         sync_cube();

//         // Copy the shared trial water into a per-thread Array so the kernel receives initialized data
//         let mut trial_copy = Array::new(consts::WATER_SIZE as u32);
//         for i in 0..consts::WATER_SIZE { trial_copy[i] = sh_trial[i]; }
...
//         // MULTITHREAD: compute trial energy
//         // ==================================
//         let e_rec = gpu_multithread::energy_for_real_water_kernel_parallel(
//             receptor_atoms, &trial_copy, n_receptor_atoms, sim_id
//         );
...
//         // ======================================
//         if UNIT_POS_X == 0 {
//             let e_wat = gpu_gcmc_moves::energy_for_real_water_with_waters_kernel(water_atoms, &trial_water, sim_id, active_waters);
//             let new_energy = e_rec + e_wat;
...
//                     let current_res = num_waters[counter_idx];
//                     let possible_resnum = current_res + 1;
//                     num_waters[counter_idx] = possible_resnum;
//                 }
//             } else if move_type == 1 && active_waters > 0 {
//                 // ---------- DELETION ----------
//                 let old_contrib = new_energy;
//                 let delta_e = -old_contrib - consts::CHEMICAL_POTENTIAL;
//                 let pref = (active_waters as f32) * f32::exp(-B);
//                 let p_acc = f32::min(pref * f32::exp(-consts::BETA * delta_e), 1.0f32);
...
//     // Persist state back to global arrays
//     if UNIT_POS_X == 0 {
//         seeds[sim_id] = rng_state;
//         num_waters[sim_id] = active_waters;
//     }
...
pub fn simulate<R: Runtime>(
    ...
    println!("Before launching the function");
    ...
    if NUM_THREADS < 2 {
        ...
        unsafe {
            run_gcmc_multithread::launch::<R>(
                ...
                ArrayArg::from_raw_parts::<u32>(&seed_tensor.handle, n_simulations*num_steps, 1), 
                ArrayArg::from_raw_parts::<u32>(&wat_num_handle, n_simulations, 1),
                ArrayArg::from_raw_parts::<f32>(&rec_partials_handle, n_simulations * CUBE_DIM_X as usize, 1),
                ArrayArg::from_raw_parts::<f32>(&wat_partials_handle, n_simulations * CUBE_DIM_X as usize, 1),
                ScalarArg {elem: receptor_atoms_buffer[receptor_atoms_buffer.len() -1] as u32}, 
                ScalarArg {elem: B},
                ScalarArg {elem: volume}, 
                ScalarArg {elem: num_steps as u32},
