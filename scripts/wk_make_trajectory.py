...
#     Args:
#         output_name (str): pdb output filename
#         molecule (parmed): parmed molecule object
#     '''
#     try:
#         molecule.save(output_name, format='pdb', overwrite=overwrite, **kwargs)
#     except IOError:
#         raise IOError("Error: file %s already exists." % output_name)

# def create_openmm_system(receptor_pdb, output_prefix, lib_files=None, frcmod_files=None):
#     """Create OpenMM system and topology

#     Args:
#         receptor_pdb (str): pdb filename
...
#     # Save topology as Amber prmtop
#     parmed_struct = pmd.openmm.load_topology(modeller.topology, 
#                                             system, 
#                                             modeller.positions)
    
#     # Save prmtop and coordinates
#     parmed_struct.save(f"{output_prefix}_system.prmtop", format='amber', overwrite=True)
#     parmed_struct.save(f"{output_prefix}_system.rst7", format='rst7', overwrite=True)
...
#     Args:
#         fname (str): output name for the trajectory
#         receptor (parmed): parmed receptor object
#         water_filenames (list): list of filenames of the water files
#     """
#     max_n_waters, idx = max_water(water_filenames)
#     min_n_waters, _ = min_water(water_filenames)
...
# def cmd_lineparser():
#     parser = argparse.ArgumentParser(description="make_trajectory_openmm")
#     parser.add_argument("-r", "--receptor", dest="receptor_filename", required=True,
#                         action="store", help="prepared receptor pdb file")
#     parser.add_argument("-w", "--dir", dest="water_directory", required=True,
#                         action="store", help="path of the directory containing the water pdb files")
#     parser.add_argument('-o', '--out', default='protein',
#                         dest='output_prefix', help='output prefix filename (default: protein)')
#     parser.add_argument('--lib', dest='lib_files', default=None, nargs='+',
#                         action='store', help='Amber lib parameter files.')
...
#     water = pmd.load_file(water_filenames[max_water(water_filenames)[1]])
#     receptor_wet = add_water_to_receptor(receptor_dry, water)
#     write_pdb_file(f"{output_prefix}_system.pdb", receptor_wet)

#     # Create OpenMM system and save topology/coordinates
#     create_openmm_system(f"{output_prefix}_system.pdb", output_prefix, lib_files, frcmod_files)

#     # Write trajectory
#     write_trajectory_file(f"{output_prefix}_system.nc", receptor_dry, water_filenames)
